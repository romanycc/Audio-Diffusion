...
class AudioDiffusionPipeline(DiffusionPipeline):
    ...
    def __init__(
        ...
        state_dict = torch.load("models/roman_cond_attention_L1/unet/diffusion_pytorch_model.bin")
...
parser.add_argument("--output_dir", type=str, default="L1_no_atten-correct")
parser.add_argument("--eval_batch_size", type=int, default=40)
parser.add_argument("--from_pretrained", type=str, default="models/roman_cond_attention_L1") #line 171 also need to modify
...
label = [code_dict[i] for i in range(50)] # 讀取類別
one_hot = torch.tensor(one_hot).squeeze()

os.makedirs(f"audio_evaluate/Predict/{args.output_dir}", exist_ok=True)
rootdir = f"audio_evaluate/Predict/{args.output_dir}"

for i in range(0,50):
    os.makedirs(f"{rootdir}/{i}", exist_ok=True)
    destdir = f"{rootdir}/{i}"

    # 全部同一類別
    one_hot = [lb.transform([i]) for j in range(args.eval_batch_size)]
    one_hot = torch.tensor(one_hot).squeeze()
    ...
    for _, audio in enumerate(audios):
        sf.write( f"{destdir}/{label[i]}_{index}.wav", audio, sample_rate)
        index += 1
...
在根目錄 python audio-diffusion/scripts/generate_eval_model.py 執行

--output_dir : wav儲存目錄,會存在 audio_evaluate/Predict/{outputdir}
--eval_batch_size : 預設40,代表一個類別生成多少不同的wav,用來根GT比較
--from_pretrained : 使用的pretrain路徑

會將50個類別都跑40筆
存在audio_evaluate/Predict/{outputdir}底下
